...
To talk to a tangerine node from inside a JavaScript application use the [web3.js](https://github.com/ethereum/web3.js) library, which gives a convenient interface for the RPC methods.
...
```bash
gtan --rpc --rpcaddr <ip> --rpcport <portnumber>
```

If accessing the RPC from a browser, CORS will need to be enabled with the appropriate domain set. Otherwise, JavaScript calls are limit by the same-origin policy and requests will fail:

```bash
gtan --rpc --rpccorsdomain "http://localhost:3000"
```

The JSON RPC can also be started from the [gtan console](https://github.com/ethereum/go-ethereum/wiki/JavaScript-Console) using the `admin.startRPC(addr, port)` command.
...
When requests are made that act on the state o tangerine, the last default block parameter determines the height of the block.
...
- `"373"`: Tangerine Network Mainnet
- `"374"`: Tangerine Network Testnet
- `"239"`: Taipei Testnet
- `"240"`: Yilan Testnet
...
Returns the curren tangerine protocol version.
...
`String` - The curren tangerine protocol version.
...
The sign method calculates an Tangerine specific signature with: `sign(keccak256("\x19Tangerine Signed Message:\n" + len(message) + message)))`.

By adding a prefix to the message makes the calculated signature recognisable as an Tangerine specific signature. This prevents misuse where a malicious DApp can sign arbitrary data (e.g. transaction) and use the signature to impersonate the victim.
...
1. `Object` - The transaction object
  ...
  - `data`: `DATA`  - The compiled code of a contract OR the hash of the invoked method signature and encoded parameters. For details see [Solidity Contract ABI](https://solidity.readthedocs.io/en/develop/abi-spec.html)
